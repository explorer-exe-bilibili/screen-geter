...
void config::init() {
	...
	else
	{
		...
		LogString = get(CAREAMTIME);
		if (wcscmp(LogString.c_str(), L"err") == 0)add(CAREAMTIME, L"10");
		LogString = get(ACTIME);
		if (wcscmp(LogString.c_str(), L"err") == 0)add(ACTIME, L"10");
		LogString = get(ENABLE2CAREMA);
		if (wcscmp(LogString.c_str(), L"err") == 0)add(ENABLE2CAREMA, L"0");
		LogString = get(OPENAC);
		if (wcscmp(LogString.c_str(), L"err") == 0)add(OPENAC, L"1");
		LogString = get(OPENCAREAM);
		if (wcscmp(LogString.c_str(), L"err") == 0)add(OPENCAREAM, L"0");
		saveFile();
		printAllConfigItems();
	}
}
...
long long config::getint(const std::wstring& name)
{
	Node* current = head;
	...
	while (current != NULL) {
		if (current->item.name == name) {
			...
			try
			{
				return stoll(current->item.value);
			}
			catch (const std::exception& e)
			{
				return 0;
